...
import com.group.project.user.SendToUsersActivity;
...
public class AdapterSendToUser extends RecyclerView.Adapter<AdapterSendToUser.MyHolder>{
    ...
    public AdapterSendToUser(Context context, List<UserModel> createModels) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        holder.itemView.setOnClickListener(view -> {
            ...
            hashMap.put("msg", SendToUsersActivity.getRoom());
            hashMap.put("isSeen", false);
            hashMap.put("timestamp", SendToUsersActivity.getRoom());
            ...
            FirebaseFirestore.getInstance().collection("users").document(FirebaseAuth.getInstance().getCurrentUser().getUid()).addSnapshotListener((value, error) -> {
                ...
                if (notify){
                    sendNotification(createModels.get(position).getId(), Objects.requireNonNull(value.get("name")).toString(), " has sent meeting invitation");
                }
                notify = false;
