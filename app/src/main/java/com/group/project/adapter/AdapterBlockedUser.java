...
public class AdapterBlockedUser extends RecyclerView.Adapter<AdapterBlockedUser.MyHolder>{

    final Context context;
    final List<UserModel> createModels;

    public AdapterBlockedUser(Context context, List<UserModel> createModels) {
            ...
            builder.setItems(colors, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                   if (which == 0){
                       ...
                       ref.child(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid()).child("BlockedUsers").child(createModels.get(position).getId()).removeValue();
                   }
                }
            });
            builder.show();
